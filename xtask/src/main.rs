...
fn release_prep(root: &Path, input: &str) -> Result<(), String> {
    ...
    let xtask_path = root.join("xtask/src/main.rs");
    ...
    let xtask_source =
        insert_release_boundary_test_stub(&read_to_string(&xtask_path)?, &version, &target_date)?;
    ...
    write_file(&xtask_path, &xtask_source)?;
...
mod tests {
    ...
    use super::{
        bundle_goldens, check_topic, contract_check, release_prep, repo_root, target_ir_goldens,
...
";
    ...
    fn release_prep_scaffolds_version_policy_changelog_and_test_stub() {
        ...
        let xtask_source = fs::read_to_string(root.join("xtask/src/main.rs")).expect("xtask");
        assert!(

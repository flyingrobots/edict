...
- digest-locked source, Core, target-profile, target-IR, lawpack, generated
  artifact, toolchain, verifier-report, and explanation references;
- logical source path validation;
- HOLMES, Watson, and Moriarty evidence binding to the selected bundle subject,
...
| BUNDLE-REQ-003 | implemented | Every hash-significant artifact reference carried by the typed bundle must be digest-locked by non-empty coordinate and valid `sha256:<64 hex>` digest review rendering. | docs/SPEC_continuum-contract-bundle-v1.md |
...
| BUNDLE-REQ-005 | implemented | HOLMES, Watson, and Moriarty evidence entries must bind to the manifest's selected bundle subject digest, target profile digest, and target IR digest. | issue #1, docs/GUIDE_edict-assurance-transparency.md |
...
| BUNDLE-TP-002 | implemented | Boundary guard | BUNDLE-REQ-003 | Removing or malforming a target-profile digest returns `InvalidArtifactReference` on the `target_profile` field. | bundle_artifact_references_must_be_digest_locked, malformed_bundle_artifact_digest_is_rejected | crates/edict-syntax/tests/contract_bundle.rs | Stable failure kind and field, not prose. |
...
| BUNDLE-TP-005 | implemented | Boundary guard | BUNDLE-REQ-005 | Omitting one of HOLMES, Watson, or Moriarty evidence returns `MissingAssuranceRole`. | holmes_watson_and_moriarty_evidence_are_required | crates/edict-syntax/tests/contract_bundle.rs | Keeps the assurance flow explicit in the typed contract. |

...
| DEVTOOLS-TP-005 | implemented | Corpus alignment | DEVTOOLS-REQ-003 | Tree-sitter accepts formatted type-call suffixes that the reference parser accepts. | tree-sitter test, tree_sitter_corpus_examples_match_reference_parser | grammars/tree-sitter-edict/test/corpus/current-subset.txt | A typed call with whitespace before `<...>` parses as a call expression with type arguments and remains accepted by `parse_module`. |

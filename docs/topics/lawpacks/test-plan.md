...
| LAWPACKS-REQ-014 | implemented | Before an Edict-authored pure helper can become a compiler fact, its inline Core parameters, local identities, ordered bindings, calls, and result must match the exported signatures, and the complete pure-helper call graph must be acyclic. The selected adapter's exact budget closure includes every helper cost template as well as operation and effect obligations. | issue #192, crates/edict-syntax/src/lawpack.rs, crates/edict-syntax/src/lawpack_adapter.rs |

...
| LAWPACKS-TP-016 | implemented | Effect-free provider configuration | LAWPACKS-REQ-008 | An adapter with no semantic effects supplies its operation profile's exact target configuration to provider invocation instead of failing configuration cardinality. | `operation_profile_configuration_is_selected_when_adapter_has_no_effects` | crates/edict-cli/src/application_build.rs | The workspace request-only fixture calibrates the generic adapter shape; Jedit #302 is the first pure executable consumer. |

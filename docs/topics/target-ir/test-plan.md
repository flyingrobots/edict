...
| TIR-REQ-005 | implemented | Target IR lowering facts can be derived from the same native target-profile facts that satisfy lowerability, so lowerability evidence and target IR generation select the same target profile and effect support. | issue #66, ROADMAP.md |

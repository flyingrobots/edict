...
| --- | --- | --- | --- | --- | --- | --- | --- |
| TIR-TP-001 | implemented | Golden path | TIR-REQ-001, TIR-REQ-002 | The supported effectful Core shape lowers to an `echo.span-ir/v1` artifact whose selected profile is `echo.dpo@1` and whose steps preserve effect coordinates, inputs, obstruction arms, and deterministic order. | supported_effectful_core_lowers_to_echo_span_ir, obstruction_arm_values_are_preserved_in_echo_span_ir | crates/edict-syntax/tests/target_ir.rs | Echo first; git-warp follows later. |

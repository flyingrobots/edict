...
| TIR-TP-026 | implemented | Boundary guard | TIR-REQ-011, TIR-REQ-012 | A Target IR requirement whose predicate or reason payload reads an earlier target step output rejects with a stable target-feature failure kind and no artifact. | requirement_that_reads_step_output_rejects_with_stable_feature_kind | crates/edict-syntax/tests/target_ir.rs | Intent-level requirements cannot depend on step-local outputs until the artifact model owns ordered or step-attached guards. |

...
| LAUTH-TP-004 | implemented | Error handling | LAUTH-REQ-004, LAUTH-REQ-005 | Invalid digests, non-integral JSON numbers, byte sentinels, duplicate or prefix-colliding paths, ownership-index descendants, unknown tagged fields, unresolved local resources, path escape, malformed pure Core, and incomplete adapters return stable failure kinds. | malformed_inputs_fail_with_stable_categories, artifact_paths_reject_file_ancestors_and_the_ownership_index_namespace, tagged_authoring_variants_reject_unknown_fields | crates/edict-syntax/tests/lawpack_authoring.rs | Assertions target structured failure kinds rather than diagnostic prose. |
| LAUTH-TP-005 | implemented | Transactional publication | LAUTH-REQ-006 | Publication replaces one owned output set, removes stale owned artifacts, restores the previous set on pre-commit failure, treats post-activation cleanup as best effort, bounds its generated index, rejects dependency inputs inside the output tree, and checks only expected output paths and bytes. | publication_replaces_the_owned_tree_and_removes_stale_files, injected_pre_activation_failure_restores_the_previous_tree, post_activation_cleanup_failure_keeps_the_committed_success, generated_ownership_index_rejects_its_own_read_limit, dependency_inputs_inside_the_owned_output_tree_reject_before_reading, check_only_detects_exact_tree_drift, check_only_rejects_an_unexpected_file_before_reading_it | crates/edict-cli/src/lawpack_build.rs | The persistent sibling lock is outside the replaceable output directory; activation is the commit point. |

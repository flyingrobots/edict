...
| RESULT-PROJ-TP-008 | planned | Property | RESULT-PROJ-REQ-003 | Sixty-four fixed-seed permutations of one canonical map produce identical canonical bytes. | `canonical_maps_are_insertion_order_independent_for_fixed_seed_cases` | crates/edict-syntax/tests/result_projection.rs | Fixed seed: `0x17305eedcafebabe`; the test must preserve insertion order until it reaches the canonical encoder. |

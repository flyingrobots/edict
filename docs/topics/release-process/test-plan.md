...
| RELEASE-REQ-008 | implemented | Release dates recorded in the structured release policy, `CHANGELOG.md`, and release notes are reconciled against the git tags that published them, rather than against each other. | CHANGELOG.md, docs/topics/release-process/policy.toml, xtask/src/release_dates.rs |
...
- `target_date` now records the actual tag date rather than a planned date, but
  ...
  the next release rather than the biweekly increment. `cargo xtask
  release-dates` will report the resulting drift once the tag exists, so this
  fails loudly at release time rather than silently.

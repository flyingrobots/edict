...
| RELEASE-TP-020 | implemented | Consistency guard | RELEASE-REQ-025 | Every `[release_notes.*]` block parses with a unique section and tag, an ISO `target_date`, a known status, and `scope`/`non_goals` lists, and no block retains scaffold placeholders, regardless of published, planned, or prep status. | release_policy_blocks_are_structurally_complete | docs/topics/release-process/policy.toml | Replaces eleven near-duplicate per-release guards with one data-driven check. |
...
| RELEASE-TP-025 | implemented | Historical coverage | RELEASE-REQ-025 | The structural guard accepts an additional completed prep block and still detects a missing historical tag when a new block keeps the total count unchanged. | release_policy_structure_accepts_additional_completed_prep, release_policy_history_cannot_be_replaced_by_a_new_release | xtask/src/tests.rs, docs/topics/release-process/policy.toml | Historical identity coverage is independent of the total number of blocks. |
| RELEASE-TP-026 | implemented | Structured findings | RELEASE-REQ-008 | Reconciliation exposes stable finding kinds, tag and surface identities, and expected/actual values without requiring callers to parse diagnostic prose. | release_date_findings_identify_policy_errors_without_prose, release_date_findings_identify_missing_tagger_date | xtask/src/release_dates.rs, xtask/src/tests.rs | Human text is rendered only at the command boundary; existing drift/gap classification is retained. |
| RELEASE-TP-027 | implemented | Date input grammar | RELEASE-REQ-024 | Only valid fixed-width ASCII YYYY-MM-DD dates are accepted; malformed syntax and calendar dates return InvalidIsoDate. | release_prep_rejects_noncanonical_iso_date, release_prep_date_validation_has_one_stable_failure | xtask/src/release_prep.rs | Includes the previously accepted 2026-1-001 form, invalid leap days, signed fields, whitespace, and non-ASCII digits. |
| RELEASE-TP-028 | implemented | Injected clock | RELEASE-REQ-024 | A supplied clock determines the default UTC date at the epoch, leap day, and end of a UTC day; pre-epoch and noncanonical-year clocks have stable failures, while an explicit date takes precedence. | release_prep_default_date_uses_supplied_utc_clock, release_prep_clock_failures_are_structured, release_prep_explicit_date_overrides_clock | xtask/src/release_prep.rs | SystemTime is read only at the CLI boundary. |

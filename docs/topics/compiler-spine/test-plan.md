...
| CSPINE-TP-024 | implemented | Golden path, error handling, and mutation sensitivity | CSPINE-REQ-003, CSPINE-REQ-004, CSPINE-REQ-023 | A pure ternary with compatible bounded branches lowers to a Core `if` expression, swapping the branches moves canonical Core identity, and incompatible branches reject with stable `TypeMismatch` identity. | pure_conditional_expression_lowers_to_core, pure_conditional_branch_mutation_moves_core_digest, pure_conditional_expression_rejects_incompatible_branches | crates/edict-syntax/tests/compiler_spine.rs | Pure ternaries, effectful branch-yield, and statement control flow remain distinct Core shapes. |
...
| CSPINE-TP-028 | implemented | Golden path and authority boundary | CSPINE-REQ-003, CSPINE-REQ-004, CSPINE-REQ-027 | A coordinate-bounded loop lowers with the canonical bound coordinate only when an exact fact supplies its numeric maximum; missing facts reject before Core. | digest_bound_coordinate_loop_cap_lowers_to_core, missing_coordinate_loop_cap_rejects_before_core, exact_lawpack_constant_enters_loop_bound_compilation | crates/edict-syntax/tests/compiler_spine.rs, crates/edict-syntax/tests/lawpack.rs | The canonical coordinate is semantic identity; the numeric value is used only for static proof and budget admission. |

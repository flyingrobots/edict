...
| CSPINE-REQ-019 | planned | The compiler spine recognizes the source `I32`, `I64`, `U32`, and `U64` scalar types, preserves an explicitly suffixed literal's exact width and signedness, and rejects out-of-range or cross-width values before Core lowering. | docs/SPEC_edict-language-v1.md |
| CSPINE-REQ-020 | planned | An explicit source basis expression is type-checked only in the pure pre-body input environment and is preserved structurally in the lowered Core intent; `basis none` remains the explicit no-basis posture. | docs/SPEC_edict-language-v1.md, EDICT-LANG-BASIS-PURE-001 |
| CSPINE-REQ-021 | planned | The compiler spine accepts a statically bounded `Bytes<max=N>` field in an operation input and preserves its exact bound in Core; unbounded bytes remain a surface error. | docs/SPEC_edict-language-v1.md |
...
| CSPINE-TP-018 | planned | Golden path | CSPINE-REQ-003, CSPINE-REQ-004, CSPINE-REQ-019 | The operation prerequisite fixture lowers `U64` input fields and explicitly suffixed values as `U64` Core types and values without `i32` or `I64` narrowing. | - | fixtures/lang/operations/explicit-basis-u64.edict | Application-neutral witness for the fixed-width coordinate domain required downstream. |
| CSPINE-TP-019 | planned | Boundary guard | CSPINE-REQ-002, CSPINE-REQ-007, CSPINE-REQ-019 | Out-of-range `U64` literals and cross-width integer assignments reject in type checking with stable structured failure kinds. | - | - | Integer width and signedness are semantic identity, not coercion hints. |
| CSPINE-TP-020 | planned | Golden path | CSPINE-REQ-003, CSPINE-REQ-004, CSPINE-REQ-020 | The operation prerequisite fixture lowers `basis input.basis` to the exact Core field-expression tree, while a basis that references a body local rejects before Core exists. | - | fixtures/lang/operations/explicit-basis-u64.edict | Basis authoring remains distinct from runtime basis resolution or admission. |
| CSPINE-TP-021 | planned | Golden path | CSPINE-REQ-003, CSPINE-REQ-004, CSPINE-REQ-021 | The operation prerequisite fixture lowers its replacement field as bounded Core `Bytes` with the authored maximum unchanged. | - | fixtures/lang/operations/explicit-basis-u64.edict | Adds only the bounded byte payload required by the downstream operation seam. |

...
| CLI | user, integrator, maintainer | current JSONL `check` smoke path | current CLI topic | current CLI topic and compiler-settings JSON Schema | current CLI topic | current structured stderr diagnostics | current CLI test plan |

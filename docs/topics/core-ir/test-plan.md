...
| COREIR-REQ-022 | implemented | Core byte types may carry an optional minimum in addition to their required maximum; exact source lengths lower as equal minimum and maximum, while max-only types omit the minimum and retain their prior canonical shape. | issue #200, docs/abi/edict-core.cddl |
| COREIR-REQ-023 | implemented | Every byte interval coordinate emitted by Core type lowering resolves back to the same bounded Core type, including distinct minimum and maximum bounds. | issue #201 |
| COREIR-REQ-024 | implemented | Structural Core type compatibility accepts the same maximum type depth as imported compiler type resolution and fails closed beyond that shared bound. | issue #201 |
| COREIR-REQ-025 | implemented | Every Core type reference classifies without module state as intrinsic, canonical self-describing structural syntax, or a named reference. Canonical structural records use sorted identifier fields and recursively canonical child references; malformed, duplicate, non-canonical, or over-depth constructors fail closed. | issue #201 |
| COREIR-REQ-026 | implemented | Intrinsic and canonical structural references resolve from their own fixed syntax and may not appear as `core.types` keys. Only named definitions occupy the table, so callers cannot shadow or redefine self-describing Core identity before canonical encoding or Target artifact construction. | issue #201 |
...
| COREIR-TP-030 | implemented | Scalar interval identity and compatibility | COREIR-REQ-002, COREIR-REQ-012, COREIR-REQ-022 | Exact byte types encode both equal interval bounds and move Core identity relative to max-only bytes; max-only byte types continue omitting `min` from canonical Core; a minimum above the maximum rejects before identity. | exact_bytes_and_imported_nominal_aliases_preserve_core_identity, max_only_bytes_preserve_prior_canonical_shape, invalid_byte_interval_rejects_before_core_identity | crates/edict-syntax/tests/compiler_spine.rs, crates/edict-syntax/tests/canonical_encoding.rs | Exact byte length is structural Core meaning; application nominal identity remains in the referenced lawpack coordinate. |
| COREIR-TP-031 | implemented | Scalar interval resolution | COREIR-REQ-002, COREIR-REQ-022, COREIR-REQ-023 | A valid pure binding typed with `Bytes<min=M,max=N>` resolves both bounds and lowers, while values outside the interval remain rejected. | ranged_core_byte_coordinates_lower_as_builtin_types | crates/edict-syntax/tests/target_ir.rs | The type coordinate is compiler-produced structural meaning, not application vocabulary. |
| COREIR-TP-032 | implemented | Type compatibility depth | COREIR-REQ-002, COREIR-REQ-024 | A pure binding copied through a 128-level structural Core type chain remains compatible and lowers, matching the accepted compiler boundary rather than rejecting at the legacy depth of 64. | `target_lowering_accepts_compiler_type_depth_boundary` | crates/edict-syntax/tests/target_ir.rs | The shared bound remains finite so cyclic or over-depth caller-built Core fails closed. |
| COREIR-TP-033 | implemented | Canonical structural references | COREIR-REQ-002, COREIR-REQ-024, COREIR-REQ-025 | Canonical structural record references round-trip through one Core parser and renderer, including nested records and records inside lists; unsorted or duplicate fields, malformed constructors, and depth beyond the shared bound reject. | `canonical_structural_type_references_round_trip` | crates/edict-syntax/src/core_ir.rs | The executable grammar, not a module table entry, owns structural identity. |
| COREIR-TP-034 | implemented | Self-describing identity boundary | COREIR-REQ-012, COREIR-REQ-025, COREIR-REQ-026 | Caller-authored `core.types` entries keyed by an intrinsic or canonical structural reference reject before canonical Core bytes and before Target IR artifacts. | `core_type_table_rejects_self_describing_reference_keys` | crates/edict-syntax/tests/canonical_encoding.rs, crates/edict-syntax/tests/target_ir.rs | Named definitions cannot shadow or redefine fixed or self-describing Core identity. |

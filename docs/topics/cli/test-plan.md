...
| CLI-REQ-017 | implemented | A `build` request selecting `lawpack` authors canonical manifest, exports, adapters, local resources, and digest sidecars through the public validators; write mode transactionally replaces one marked owned tree, while `checkOnly` detects complete-tree drift without writing. | issue #195 |
...
| CLI-TP-033 | implemented | Lawpack publication recovery | CLI-REQ-017 | Variable-size owned output replacement removes stale files, an injected failure before activation restores the exact previous tree, and check-only compares the complete tree. | publication_replaces_the_owned_tree_and_removes_stale_files, injected_pre_activation_failure_restores_the_previous_tree, check_only_detects_exact_tree_drift | crates/edict-cli/src/lawpack_build.rs | The persistent lock is outside the replaceable output directory. |

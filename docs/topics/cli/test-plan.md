...
| CLI-REQ-012 | implemented | The checked-in CLI golden corpus can be regenerated by `cargo xtask cli-goldens --write` and checked by `cargo xtask cli-goldens --check`; `cargo xtask verify` runs the check mode. | xtask/src/main.rs |
...
| CLI-TP-018 | implemented | Golden maintenance | CLI-REQ-012 | `cargo xtask cli-goldens --check` replays every checked-in CLI golden case and fails on stdout, stderr, or exit-code drift; `cargo xtask cli-goldens --write` regenerates those artifacts without producing a diff when the corpus is current. | cli_goldens_command_is_wired_into_verify | fixtures/cli/README.md | Makes intentional CLI stream-contract changes reviewable as one generated fixture diff. |

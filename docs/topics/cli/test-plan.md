...
| CLI-REQ-016 | implemented | The explicit external-action `build` route validates a request-only source, complete root-reachable capability/adapter/target-profile closure, and exact canonical schema/reconciliation resource closure; rejects profile-budget mismatches, disconnected lawpacks, and missing, duplicate, disconnected, substituted, opaque, non-canonical, placeholder, or over-budget resources; invokes no provider component; and atomically publishes the owning encoders' exact Core and Target IR bytes while clearing stale executable outputs. | issue #180 |

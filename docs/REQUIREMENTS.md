...
| EDICT-CORE-PREIMAGE-LIST-001 | Canonicalization rules carry a positive exhaustive Core-action preimage inclusion list | Language | `core/preimage/inclusion-list` | `core/preimage/scavenger-hunt` | spec |
...
| EDICT-LANG-ACTION-CLAUSES-001 | Action clause requiredness (at least one of profile/implements — both allowed; budget; basis-unless-template); omission parseable but semantically rejected | Language | `lang/clauses/required-present` | `lang/clauses/missing-budget` | spec |
